...
def check_for_newer_version(pkg_vers, pypi_ver):
    '''Check if dict value(s) in pkg_vers are older than dict values in pypi_ver,
       any one older value indicates they should be processed
       
       pkg_vers is string "pkg==ver,pkg==ver" or "none",
       pypi_ver is [{"package": "pkg", "version": "ver"}, ...] 
       '''
    ...
    try:
        current = dict(s.split('==') for s in pkg_vers.split(','))
    except ValueError:
